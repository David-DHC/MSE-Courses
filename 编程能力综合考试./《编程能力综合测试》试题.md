...
给定一个长度为N的整数数组，对每个数按以下规则执行操作，共执行K次：
...
给定一个长度为N的整数数组，其中数字范围为[0,100]。对数组进行以下规则的排序：
...
给定一个维度为N、只包括1和0两个数的方阵，定义“块”为通过上、下、左、右任一维度包含1而连接起来的图形，且图形中1的个数称为块的面积。例如：
```math 
\begin{bmatrix}1&1&0\\0&1&0\\1&0&1\end{bmatrix}
```
的左下、右下角均有面积为1的块，而上方有面积为3的块。

对于输入的方阵，允许进行最多1次操作，将一个0转为1。求在完成输入和操作后，方阵理论上可能达到的最大块的面积。

### 输入

- 第一行：一个数，代表维度N。N\leq 500。
- 其后N行，每行N个数：代表输入的方阵。
